		...
		<div id="list-message" class="content scaffold-list" role="main">
			...
			<thead>
					<tr>
						...
						<th><g:message code="message.person.label" default="Person" /></th>
				...
				<g:each in="${messageInstanceList}" status="i" var="messageInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="_show" id="${messageInstance.id}">${fieldValue(bean: messageInstance, field: "message")}</g:link></td>
					
						<td><g:formatDate date="${messageInstance.dateCreated}" /></td>
					
						<td>${fieldValue(bean: messageInstance, field: "person")}</td>
